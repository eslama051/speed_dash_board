    ...
    getpromoCodeToEdit() {
      server
        ...
        .then((res) => {
          ...
          this.discountPercentage = res.data.data.discount_percentage;
          this.startDate = res.data.data.start_date;
          this.endDate = res.data.data.expire_date;
          this.numberOfUse = res.data.data.number_of_use;
        });
    },
    submitForm() {
      this.isLoading = true;
      ...
      server
        .post(`/dashboard/coupon/${this.id}`, formData, {
          headers: {
            Authorization: `Bearer ${this.$store.getters["auth/token"]}`,
          },
        })
