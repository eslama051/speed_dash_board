...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        {
          text: "اسم الحاله (عريي)",
          value: "ar.name",
          //   sortable: false,
        },
        {
          text: "اسم الحاله (انجيزي)",
          value: "en.name",
        },
  ...
  methods: {
    ...
    deleteItemConfirm() {
      server
        .delete(`/dashboard/productStatus/${this.deleteId}`, {
          headers: {
    ...
    editItem(id) {
      this.$router.push(`/status/edit/${id}`);
    },
    getitesmPerPage() {
      this.isLoading = true;
      server
        .get(`/dashboard/productStatus?page=${this.page}`, {
          headers: {
            Authorization: `Bearer ${this.$store.getters["auth/token"]}`,
          },
        })
